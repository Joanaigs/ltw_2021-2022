...
class Image{
    ...
    static public function getImage(PDO $db, string $id, Session $session): array{
        $stmt = $db -> prepare('
    ...
    static function addDish(PDO $db, string $name, string $price, string $idMeal, int $idRest, string $idType, int $idImage)  {
    ...
    static function removeDish(PDO $db, string $id)  {
        $stmt = $db->prepare('DELETE FROM Images where id=(select image from Dish where id=?)' );
        $stmt->execute(array($id));
        $stmt = $db->prepare('DELETE FROM Dish where id=?' );
        $stmt->execute(array($id));
        $filePath="images/dishes/$id.jpg";
